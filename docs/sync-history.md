...
| Route                | Purpose                                                  |
| -------------------- | -------------------------------------------------------- |
| /sync-history        | Browse and filter sync runs.                             |
...
1. User selects **Sync all** in the application header.
...
4. Sync run detail is the only place for live progress, per-channel outcomes,
   ...
   status is returned.
5. If creation conflicts, the header action shows the mapped feedback. The
   current contract does not return an active run ID, so it does not expose a
   fake **View active run** action.
