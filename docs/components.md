...
| FilterToolbar | `app-filter-toolbar`  | `ariaLabel`, `stackOnMobile = input(false)`, `collapseOnMobile = input(false)`, `activeFilterCount = input(0)` | —                                  | Layout chung cho list filter; `collapseOnMobile` hiển thị nút Filters dưới 600px và mở field ngay bên dưới; content là `mat-form-field`/`mat-select` của feature          |
